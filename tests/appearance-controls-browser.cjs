...
(async () => {
  ...
  try {
    ...
    for (const [id, value] of [['camera-roll', '10'], ['elevation', '5'], ['distance', '40'], ['cloud-radius', '120'], ['framing', '-1'], ['framing-y', '5']])
    ...
    results.brightness = await page.evaluate(() => {
      ...
      d.applySettings({brightnessMin: .65, brightnessMax: 1.4});
    ...
    await page.evaluate(() => {
      const d = GravityDemo; d.applySettings({cloudThickness: 1}); d.simTime = 0; d.allocateParticles(); d.updateParticles(0, d.fadeSeconds);
      window.readShape = () => {
    ...
    const valid = await page.evaluate(() => [GravityDemo.cloudRadius, GravityDemo.brightnessMin, GravityDemo.brightnessMax, GravityDemo.cloudThickness]);
    await page.evaluate(() => GravityDemo.applySettings({cloudRadius: NaN, brightnessMin: Infinity, brightnessMax: NaN, cloudThickness: NaN}));
    assert.deepEqual(await page.evaluate(() => [GravityDemo.cloudRadius, GravityDemo.brightnessMin, GravityDemo.brightnessMax, GravityDemo.cloudThickness]), valid);
    assert.deepEqual(errors, []);
    results.passed = true;
    console.log('PASS: brightness, radius, orbital thickness, baseline restoration and live controls.');
  } finally {
