...
(async()=>{
  ...
  try{
    ...
    await page.evaluate(()=>{GravityDemo.paused=true;GravityDemo.palette.hsv.animated=false;GravityDemo.palette.custom.animated=false;});
    ...
    const stopped=await page.evaluate(()=>GravityDemo.palette.custom.offset);await page.waitForTimeout(200);
    assert.equal(await page.evaluate(()=>GravityDemo.palette.custom.offset),stopped,'Pause freezes palette drift');
    await page.evaluate(()=>{GravityDemo.paused=false;GravityDemo.speed=0;});await page.waitForTimeout(700);
    const moved=await page.evaluate(()=>GravityDemo.palette.custom.offset);assert.ok(moved>stopped);
    await set('palette-speed',-1);await page.waitForTimeout(350);assert.ok(await page.evaluate(v=>GravityDemo.palette.custom.offset<v,moved));
    await page.locator('#palette-animate').uncheck();const held=await page.evaluate(()=>GravityDemo.palette.custom.offset);
    await page.waitForTimeout(200);assert.equal(await page.evaluate(()=>GravityDemo.palette.custom.offset),held);
    ...
    await page.locator('#color-mode').selectOption('hsv');assert.equal(await page.evaluate(()=>GravityDemo.palette.hsv.offset),.5);
    await set('palette-offset',0);await set('palette-speed',1);await page.locator('#palette-animate').check();
    await page.waitForTimeout(350);assert.ok(await page.evaluate(()=>GravityDemo.palette.hsv.offset>0));
