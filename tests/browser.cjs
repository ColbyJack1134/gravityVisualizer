...
(async()=>{
  ...
  try{
    ...
    assert.deepEqual(await page.evaluate(()=>[GravityDemo.spin,GravityDemo.exposure,GravityDemo.speed,GravityDemo.material]),[.5,1.7,10,.7]);
    await page.evaluate(()=>GravityDemo.applySettings({roll:0,framing:0,framingY:0,cloudRadius:22,cloudThickness:.15,elevation:18}));await ready();
