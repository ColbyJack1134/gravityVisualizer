...
(async () => {
  ...
  try {
    ...
    assert.equal(defaults.tilt, 10); assert.equal(defaults.elevation, 5); assert.equal(defaults.stats.cameraDistance, 40);
    ...
    await apply({floatingcamera: true, continuoustracing: false, cameramotion: 'fixed'}); await ready();
    assert.equal(await page.evaluate(() => GravityWallpaper.renderer.floatingCamera), false);
    await apply({continuoustracing: true});
    assert.deepEqual(await page.evaluate(() => {
      const r = GravityWallpaper.renderer;
      return [r.continuousTracing, r.floatingCamera, r.job];
    }), [true, true, null]);
    const orbitBuilds = await page.evaluate(() => GravityWallpaper.renderer.cacheBuilds);
    await apply({orbittilt: 75, orbitrotation: 135, orbitnear: 38, orbitfar: 60, orbitspeed: -4.5});
    assert.deepEqual(await page.evaluate(() => {
      const r = GravityWallpaper.renderer;
      return [r.orbitTilt, r.orbitRotation, r.orbitNear, r.orbitFar, r.orbitSpeed, r.cacheBuilds];
    }), [75, 135, 38, 60, -4.5, orbitBuilds]);
    const heldCamera = await page.evaluate(() => GravityWallpaper.renderer.cameraView());
    await page.mouse.move(400, 100); await page.mouse.down();
    await page.mouse.move(430, 120); await page.mouse.up(); await page.mouse.wheel(0, 100);
    assert.deepEqual(await page.evaluate(() => GravityWallpaper.renderer.cameraView()), heldCamera);
    await apply({continuoustracing: false}); await ready();
    assert.equal(await page.evaluate(() => GravityWallpaper.renderer.floatingCamera), false);
    results.checks.push('Continuous tracing gates floating camera, orbit sliders apply live and wallpaper ignores mouse camera input');
