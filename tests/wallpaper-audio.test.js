...
spectrum.silenceThreshold = .001; run(10, samples(4, 0.00001));
assert.equal(spectrum.driven, 0); assert.equal(Math.max(...spectrum.levels), 0);
spectrum.silenceThreshold = 0; run(0.6, samples(4, 0.00001));
...
spectrum.autoSensitivity = true; spectrum.silenceThreshold = .001;
run(20, samples(4, .03)); assert.ok(spectrum.signalPresent);
spectrum.silenceThreshold = .003;
run(10, samples(4, .03));
assert.equal(spectrum.driven, 0, 'Adjustable cutoff filters continuous host noise before automatic gain');
assert.ok(spectrum.energy < 1e-10);
spectrum.autoSensitivity = false; spectrum.silenceThreshold = 0;
